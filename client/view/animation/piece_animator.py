...
class PieceAnimator:
    ...
    def update(self, dt_ms: int, snapshot) -> None:
        ...
        for piece_snapshot in snapshot.pieces:
            ...
            self._advance_if_finished(piece_snapshot, tracked, snapshot)

    def _advance_if_finished(self, piece_snapshot, tracked, snapshot) -> None:
        ...
        if tracked.elapsed_in_state_ms >= duration_ms:
            ...
            rest_deadline = snapshot.resting_until.get(piece_snapshot.id)
            tracked.motion_duration_ms = (
                max(0, rest_deadline - snapshot.server_time_ms)
                if next_state in {"long_rest", "short_rest"}
                and rest_deadline is not None
                else None
            )
