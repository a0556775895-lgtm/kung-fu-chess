...
> **הערה היסטורית:** זהו מסמך תכנון המתעד את שלבי הפיתוח ואת מבנה הפרויקט בזמן שבו התקבלו ההחלטות. לכן מופיעים בו נתיבים ישנים ומצב מקומי שכבר הוסר. המבנה הפעיל כיום הוא `client/`, ‏`server/`, ‏`networking/` ו-`DOCS/`; הוראות ההרצה והמבנה העדכניים נמצאים ב-[README.md](../../README.md).
...
- **Serialization** (`networking/serializers/snapshot.py`): גבול תעבורה מפורש שממיר `GameSnapshot` ל-JSON ובחזרה. הוא משותף לשרת וללקוח, אינו מכיל חוקי משחק ואינו תלוי ב-WebSocket, כך שפורמט ה-snapshot מוגדר ונבדק במקום יחיד.
...
│   ├── services/                      [חדש D–F] auth/session_registry/elo/matchmaker/rooms
...
| B4.1 — לקוח רשת ברקע | הושלם ואושר | `NetworkClient` מבצע JOIN, שומר את הקונפיג וה-snapshot הראשוני, ומעביר הודעות בין thread של WebSocket ל-thread של התצוגה באמצעות תורים; חיבור, פקודה וסגירה נבדקו מול שרת אמיתי; כל 193 הבדיקות עברו |
...
- `networking/serializers/snapshot.py` הוא המקור היחיד לפורמט ה-JSON של `STATE`: הוא מבצע המרה דו-כיוונית, כולל `schema_version`, ולידציית שדות ו-round-trip. `networking/protocols/game.py` עוטף את ה-JSON במעטפת הטקסט בלבד; השרת והלקוח אינם מרכיבים payload ידנית.
...
| תת-שלב | סטטוס | תוצאה |
|---|---|---|
| C1 — חוזה Login ושמות פעילים | הושלם ואושר | `login_protocol.py` מגדיר `LOGIN`/`LOGIN_OK` וולידציית שם נפרדת מפרוטוקול המשחק; `ActiveUserRegistry` שמר שמות פעילים בזיכרון ומנע כפילות ללא תלות באותיות גדולות, ובהמשך הוחלף ב־E1 על ידי `SessionRegistry`; כל 240 הבדיקות עברו |
...
| D3 — פרוטוקול Register/Login ושילוב בלקוח | הושלם ואושר | `networking/protocols/auth.py` מגדיר `REGISTER`/`LOGIN`/`AUTH_OK`; ה-CLI קולט סיסמה מוסתרת; `GameServer` מפעיל `AuthService` מול SQLite לפני `JOIN`; זהות DB ושם תצוגה נשמרים בנפרד. כל 273 הבדיקות עוברות |
...
| D6 — שמירת סיום משחק אטומית | הושלם ואושר | לכל `Match` נוסף מזהה מופע ייחודי; `GameResult` מחזיק מנצח, סיבה ומשך משחק במילישניות. `Match.finish()` מנהלת את סדר הסיום: שמירת המשחק ושני עדכוני ELO בטרנזקציה אחת, סימון התוצאה ורק אז שליחת `GAME_OVER` ללקוחות. ניסיון שמירה חוזר אינו משנה שוב את הדירוגים. כל 385 בדיקות הפרויקט עוברות בכיסוי לוגיקה נמדדת של 100% |
...
- **שינוי BLL קטן**: `GameEngine` מדווח מי ניצח דרך `winner_color` בלי לשנות את Protocol `on_game_over()`; `server/game/match.py` יוצר את תוצאת המשחק ו-`GameCompletionService` מפעילה את `calculate_elo()` ושומרת דרך ה-DAL.
- תוצאת משחק היא אובייקט מפורש הכולל `winner_color`, סיבה (`KING_CAPTURE`/`RESIGN`/`DISCONNECT`) ומשך משחק במילישניות. כל מסלולי הסיום עוברים דרך `Match.finish(result)` אידמפוטנטי, כדי ששמירת המשחק ועדכון ELO יתבצעו פעם אחת בלבד ובטרנזקציה אחת, לפני שליחת `GAME_OVER` ללקוחות.
...
| E2 — Matchmaker דרך JOIN הקיים | הושלם ואושר | `JOIN` מכניס לתור המחולק ל־rating buckets; טווח הדירוג וזמן ההמתנה נקבעים ב־`server/config.py` ומוזרקים ל־`Matchmaker`. החיפוש בוחר את הדירוג הקרוב ביותר ואז את הממתינה הוותיקה ביותר. כל זוג יוצר `Match` מבודד עם `game_id` ייחודי ב־`GameRegistry`; הראשונה קובעת את הקונפיג, ניתוק מסיר מהתור, ופקיעת זמן ההמתנה מסתיימת ב־`ERR match_timeout`. ארבע שחקניות יכולות כעת לשחק בשני משחקים מקבילים במקום לקבל `server_full` |
| E3 — ניתוק, countdown ו-reconnect בשרת | הושלם ואושר — E3.1–E3.4 | כל `JOIN` נושא token חובה ומנותב לפי מצב ה־Session ל־Matchmaker או ל־`ReconnectService`. השירות שומר את המקום, הצבע והזהות, ומשחזר חיבור לאותו `Match`. בניתוק המשחק כולו מושהה: זמן השרת, תנועות ופקודות נעצרים. חזרה בתוך חלון החסד המוגדר בקונפיג מבטלת את טיימר ההפסד וממשיכה מאותה נקודה; אם החלון פג, היריבה מנצחת ב־`DISCONNECT` ונשלח snapshot סופי עם צבע המנצחת. שתי מנותקות נשמרות בנפרד והמשחק מתחדש רק לאחר שכל המנותקות חזרו. כל 433 בדיקות הפרויקט עוברות בכיסוי לוגיקה נמדדת של 100% |
| E4 — Reconnect בלקוחה ובדיקות E2E | הושלם ואושר | `NetworkClient` שומרת את אותו thread ומחליפה רק את ה־WebSocket, מבצעת `JOIN` עם ה־token הקיים ללא login נוסף, חוסמת פקודות בזמן reconnect ומציגה countdown בחלון הקיים. היריבה רואה הודעת ניתוק משלה, snapshot מלא מעדכן את אותו Proxy לאחר החזרה, וסגירה יזומה אינה מפעילה reconnect. בדיקות E2E מפילות socket אמיתי, מאמתות שני reconnects רצופים לאותו `Match` ובודקות כישלון לאחר פקיעת חלון החסד. כל 449 בדיקות הפרויקט עוברות בכיסוי לוגיקה נמדדת של 100% |

- נשתמש ב-`JOIN` ובמעטפת `EVENT` הקיימים ולא נוסיף פקודת `PLAY`.
- ספריית WebSocket תישאר אחראית לזיהוי אובדן הקשר; לא ייבנה מנגנון ping/pong מקביל.
...
- `engine/game_engine.py`, `server/game/controller.py`, `networking/protocols/game.py`, `server/game/game_registry.py`, `server/transport/broadcaster.py` (ליבת השלב הרשתי, כולל בידוד בין משחקים)
...
- **חריגה מודעת בשכבתיות**: `client/local_session.py` (מצב hot-seat מקומי) עוקף לגמרי את שכבת ה-Controller ומדבר ישירות מול ה-BLL (`engine.game_engine.GameEngine`) — באותו תהליך, בלי socket, בלי `networking/protocols/game.py`, בלי `server/game/controller.py`. זה **מכוון**, לא פספוס: המטרה היא ששחקן יחיד/hot-seat לא יזדקק לתהליך שרת נפרד כדי לשחק. אך זו בהחלט חריגה מעקרון "כל בקשה עוברת Controller" שמוחל בנתיב הרשתי — מתועד כאן במפורש כדי שלא "יתגלה" כהפתעה בסקירת קוד.
