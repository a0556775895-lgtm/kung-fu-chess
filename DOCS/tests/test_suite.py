...
from server.rules.piece_factory import PieceFactory
from server.rules import piece_rules
from server.rules.rule_engine import RuleEngine, MoveValidation
from server.realtime.motion import Motion
from server.realtime.real_time_arbiter import RealTimeArbiter, ArrivalEvents
...
def _make_controller(lines):
    ...
    return InputController(board, engine, mapper), board, engine
...
class _RecordingObserver:
    ...
    def on_jump_started(self, piece, position, duration_ms):
        self.jump_started.append((piece, position, duration_ms))
...
def test_game_engine_subscribe_receives_jump_started():
    ...
    _, position, duration_ms = observer.jump_started[0]
    assert position == Position(0, 0)
    assert duration_ms == piece_rules.get_airborne_duration()
