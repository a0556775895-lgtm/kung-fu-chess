...
TEST_TEMP_DIRECTORY = Path(__file__).resolve().parent / "temp"


@pytest.fixture
def sqlite_path():
    """Provide one ignored SQLite path and remove every sidecar after the test."""
    TEST_TEMP_DIRECTORY.mkdir(parents=True, exist_ok=True)
    database_path = TEST_TEMP_DIRECTORY / f"kfc_test_{uuid.uuid4().hex}.db"
    try:
        yield database_path
    finally:
