...
| D4 — תוצאת משחק מפורשת | מומש וממתין לאישור | `GameResult` שומר מנצח, סיבת סיום וזמן UTC; תפיסת מלך מתנקזת ל-`Match.finish()` פעם אחת; המנצח נכלל ב-snapshot ומוצגת תמונת הסיום המתאימה בלקוח. כל 279 הבדיקות עוברות |
...
| D — Auth + SQLite + ELO | בתהליך — D1–D3 אושרו, D4 מומש וממתין לאישור | register/login מאובטחים; rating מתחיל ב-1200; סיום משחק מעדכן DB ו-ELO פעם אחת ובטרנזקציה אחת |
